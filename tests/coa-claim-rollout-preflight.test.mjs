...
const forwardCorrectionSql = runbook.match(
  /correction_reason=\$FORWARD_CORRECTION[\s\S]*?<<'SQL'\n([\s\S]*?)\nSQL/,
)?.[1] ?? '';

function reportMutationFingerprint(database, reportId) {
  const result = runPsql(`
SELECT jsonb_build_object(
    'report', to_jsonb(report),
    'audit_rows', (
        SELECT COUNT(*) FROM public.audit_logs
        WHERE table_name = 'coa_reports' AND record_id = report.id
    )
)::text
...
test('Issue 74 Docker integration is isolated in one disposable database', async (t) => {
  ...
  try {
    ...
    for (const [variable, value] of [
      ...
      await t.test(`forward correction rejects blank ${variable}`, () => {
        const before = reportMutationFingerprint(database, fixture.report_id);
        const variables = {
          report_id: fixture.report_id,
          accountable_user_id: fixture.accountable_user_id,
          correction_reason: 'Approved correction',
          approval_reference: 'CHG-2026-074-CORRECTION',
          [variable]: value,
        };
        assertPsqlFailed(
          runPsql(forwardCorrectionSql, variables, database),
          new RegExp(`${variable} must not be blank`),
        );
        assert.deepEqual(
          reportMutationFingerprint(database, fixture.report_id),
          before,
        );
      });
    }
    ...
    await t.test('missing seeded baseline reports an actionable prerequisite', () => {
      assertPsqlPassed(runPsql(
        `UPDATE public.coa_reports
         SET status = 'failed',
             error_message = 'Test-only missing seeded baseline'
         WHERE status = 'ready';`,
        {},
        database,
      ));
      assert.throws(
        ...
        /seeded Docker baseline.*ready.*CoA.*sample submission.*active.*analyst.*manager/is,
      );
    });
  } finally {
    if (database) {
...
test('runbook safely transports operator-entered free text', () => {
  ...
  assert.ok(forwardCorrectionSql, 'Forward-correction SQL block must be extractable');
  assert.ok(
    forwardCorrectionSql.indexOf('correction_reason must not be blank')
      < forwardCorrectionSql.indexOf('BEGIN;'),
    'Forward-correction input validation must run before BEGIN',
  );
});
