...
const migration223Path =
  'supabase/migrations/223_fix_client_resolver_role_guard.sql'
...
describe('client resolver v2 forward-only hardening migration', () => {
  ...
  it('keeps applied migration 222 byte-for-byte immutable', () => {
    const digest = createHash('sha256')
      .update(readFileSync(migration222Path))
      .digest('hex')
  ...
  it('fails closed for missing roles without coercing the role enum', () => {
    const sql = readFileSync(migration223Path, 'utf8')

    expect(sql).toMatch(
      /v_user_role\s+IS NULL\s+OR\s+v_user_role\s+NOT IN\s*\(\s*'analyst'\s*,\s*'manager'\s*\)/i,
    )
    expect(sql).not.toMatch(
      /IF\s+auth\.uid\(\)\s+IS NULL[\s\S]{0,160}COALESCE\s*\(\s*public\.get_user_role\(\)/i,
    )
  })

  it('fails closed for exact-plus-accent candidates', () => {
  ...
  it('reasserts least-privilege grants after replacing resolver functions', () => {
    ...
    expect(sql).toMatch(
      /REVOKE ALL ON FUNCTION public\.resolve_client_identity_v2[\s\S]+FROM anon/i,
    )
    expect(sql).toMatch(
