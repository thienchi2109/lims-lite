...
export function loadFixture(database) {
  const prerequisiteMessage = [
    'Issue 74 tests require the seeded Docker baseline:',
    'a ready non-deleted CoA report with signature_id,',
    'a matching sample submission, and an active analyst or manager user.',
    'Restore or reseed the standard Docker test data before running',
    'npm run test:coa-claim-rollout.',
  ].join(' ');
  const result = runPsql(`
SELECT jsonb_build_object(
    'report_id', report.id,
...
`, {}, database);
  assertPsqlPassed(result);
  const fixtureOutput = result.stdout.trim();
  assert.ok(fixtureOutput, prerequisiteMessage);
  let fixture;
  try {
    fixture = JSON.parse(fixtureOutput);
  } catch (error) {
    throw new Error(
      `${prerequisiteMessage} Fixture query returned invalid JSON.`,
      { cause: error },
    );
  }
  assert.ok(fixture.report_id, prerequisiteMessage);
  assert.ok(fixture.source_submission_id, prerequisiteMessage);
  assert.ok(fixture.accountable_user_id, prerequisiteMessage);
  return fixture;
}
