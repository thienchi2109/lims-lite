...
\! timeout --kill-after=5s 40s sh -c "psql -v ON_ERROR_STOP=1 -U postgres -d postgres -X -Atqc \"BEGIN; SET ROLE service_role; SET request.jwt.claims TO '{\\\"role\\\":\\\"service_role\\\"}'; SELECT public.create_approval_batch_server('93300000-0000-0000-0000-000000000001'::uuid, '93300000-0000-0000-0000-000000000100'::uuid, 'selected', ARRAY['93300000-0000-0000-0000-000000000010'::uuid, '93300000-0000-0000-0000-000000000011'::uuid], 'Concurrency snapshot', '93300000-0000-0000-0000-000000000008'::uuid, clock_timestamp(), 'manager_email_otp'); COMMIT;\" > /tmp/approval-batch-concurrency-a.out 2>&1 & first_pid=\$!; locked=f; for attempt in \$(seq 1 100); do locked=\$(psql -v ON_ERROR_STOP=1 -U postgres -d postgres -X -Atqc \"SELECT EXISTS (SELECT 1 FROM pg_locks WHERE locktype = 'advisory' AND objid = 193193001 AND granted);\"); [ \"\$locked\" = t ] && break; sleep 0.05; done; if [ \"\$locked\" = t ]; then insert_started=\$(date +%s%3N); psql -v ON_ERROR_STOP=1 -U postgres -d postgres -X -Atqc \"INSERT INTO public.results (id, sample_id, assay_id, value, status, entered_by, entered_at) VALUES ('93300000-0000-0000-0000-000000000021'::uuid, '93300000-0000-0000-0000-000000000010'::uuid, '93300000-0000-0000-0000-000000000006'::uuid, 'late-confidential', 'entered'::public.result_status, '93300000-0000-0000-0000-000000000003'::uuid, clock_timestamp());\" > /tmp/approval-batch-concurrency-b.raw 2>&1; second_status=\$?; insert_elapsed=\$((\$(date +%s%3N) - insert_started)); if [ \"\$second_status\" -eq 0 ] && [ \"\$insert_elapsed\" -ge 1000 ]; then printf 't\n' > /tmp/approval-batch-concurrency-b.out; else cat /tmp/approval-batch-concurrency-b.raw > /tmp/approval-batch-concurrency-b.out; printf 'f\n' >> /tmp/approval-batch-concurrency-b.out; fi; else second_status=1; fi; wait \$first_pid; first_status=\$?; [ \"\$first_status\" -eq 0 ] && [ \"\$second_status\" -eq 0 ] && grep -q '\"outcome_code\": \"BATCH_CREATED\"' /tmp/approval-batch-concurrency-a.out && grep -qx 't' /tmp/approval-batch-concurrency-b.out" && printf '0\n' > /tmp/approval-batch-concurrency-status || printf '1\n' > /tmp/approval-batch-concurrency-status
...
\! timeout --kill-after=5s 40s sh -c "psql -v ON_ERROR_STOP=1 -U postgres -d postgres -X -Atqc \"BEGIN; SELECT set_config('test.parent_batch_id', (SELECT id::text FROM public.approval_batches WHERE requested_by = '93300000-0000-0000-0000-000000000001'::uuid AND request_key = '93300000-0000-0000-0000-000000000100'::uuid), true); SET ROLE service_role; SET request.jwt.claims TO '{\\\"role\\\":\\\"service_role\\\"}'; SELECT public.retry_failed_approval_batch_server('93300000-0000-0000-0000-000000000001'::uuid, current_setting('test.parent_batch_id')::uuid, '93300000-0000-0000-0000-000000000101'::uuid, '93300000-0000-0000-0000-000000000009'::uuid, clock_timestamp(), 'manager_email_otp'); COMMIT;\" > /tmp/approval-batch-retry-concurrency-a.out 2>&1 & first_pid=\$!; locked=f; for attempt in \$(seq 1 100); do locked=\$(psql -v ON_ERROR_STOP=1 -U postgres -d postgres -X -Atqc \"SELECT EXISTS (SELECT 1 FROM pg_locks WHERE locktype = 'advisory' AND objid = 193193002 AND granted);\"); [ \"\$locked\" = t ] && break; sleep 0.05; done; if [ \"\$locked\" = t ]; then psql -v ON_ERROR_STOP=1 -U postgres -d postgres -X -Atqc \"WITH started AS MATERIALIZED (SELECT clock_timestamp() AS started_at), updated AS (UPDATE public.results SET assay_id = '93300000-0000-0000-0000-000000000006'::uuid FROM started WHERE id = '93300000-0000-0000-0000-000000000020'::uuid RETURNING 1) SELECT clock_timestamp() - (SELECT started_at FROM started) >= INTERVAL '1 second' FROM updated;\" > /tmp/approval-batch-retry-concurrency-b-update.out 2>&1 & second_pid=\$!; (insert_started=\$(date +%s%3N); psql -v ON_ERROR_STOP=1 -U postgres -d postgres -X -Atqc \"INSERT INTO public.results (id, sample_id, assay_id, value, status, entered_by, entered_at) VALUES ('93300000-0000-0000-0000-000000000023'::uuid, '93300000-0000-0000-0000-000000000011'::uuid, '93300000-0000-0000-0000-000000000005'::uuid, 'retry-late', 'entered'::public.result_status, '93300000-0000-0000-0000-000000000003'::uuid, clock_timestamp());\" > /tmp/approval-batch-retry-concurrency-b-insert.raw 2>&1; insert_status=\$?; insert_elapsed=\$((\$(date +%s%3N) - insert_started)); if [ \"\$insert_status\" -eq 0 ] && [ \"\$insert_elapsed\" -ge 1000 ]; then printf 't\n' > /tmp/approval-batch-retry-concurrency-b-insert.out; else cat /tmp/approval-batch-retry-concurrency-b-insert.raw > /tmp/approval-batch-retry-concurrency-b-insert.out; printf 'f\n' >> /tmp/approval-batch-retry-concurrency-b-insert.out; fi; exit \$insert_status) & third_pid=\$!; wait \$second_pid; second_status=\$?; wait \$third_pid; third_status=\$?; else second_status=1; third_status=1; fi; wait \$first_pid; first_status=\$?; [ \"\$first_status\" -eq 0 ] && [ \"\$second_status\" -eq 0 ] && [ \"\$third_status\" -eq 0 ] && grep -q '\"outcome_code\": \"BATCH_CREATED\"' /tmp/approval-batch-retry-concurrency-a.out && grep -qx 't' /tmp/approval-batch-retry-concurrency-b-update.out && grep -qx 't' /tmp/approval-batch-retry-concurrency-b-insert.out" && printf '0\n' > /tmp/approval-batch-retry-concurrency-status || printf '1\n' > /tmp/approval-batch-retry-concurrency-status
