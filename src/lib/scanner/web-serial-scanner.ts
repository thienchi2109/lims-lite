export const DEFAULT_SCANNER_SERIAL_IDLE_TIMEOUT_MS = 120
export const DEFAULT_SCANNER_SERIAL_BAUD_RATE = 9600
...
export type ScannerSerialFrameDecoderOptions = {
    idleTimeoutMs?: number
    onPayload: (payload: string) => void
}

export type ScannerSerialFrameDecoder = {
...
export async function getGrantedSerialPorts<TPort>(
    serialApi: GrantedPortApi<TPort>,
): Promise<TPort[]> {
    return serialApi.getPorts()
}

export function sanitizeScannerPayload(value: string): string {
    return value
        .replace(/\uFEFF/g, '')
        .replace(/[\u001c\u001d\u001e\u001f]/g, '|')
        .replace(/[\u0000-\u001b\u007f]/g, '')
...
export function createScannerSerialFrameDecoder({
    idleTimeoutMs = DEFAULT_SCANNER_SERIAL_IDLE_TIMEOUT_MS,
    onPayload,
}: ScannerSerialFrameDecoderOptions): ScannerSerialFrameDecoder {
    ...
    const emitPayload = (payload: string) => {
        const sanitizedPayload = sanitizeScannerPayload(payload)
