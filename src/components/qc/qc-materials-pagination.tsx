...
interface QCMaterialsPaginationProps {
    page: number
    pageSize: number
    total: number
}
...
export function QCMaterialsPagination({ page, pageSize, total }: QCMaterialsPaginationProps) {
    ...
    const handlePageChange = useCallback((newPage: number) => {
        ...
        updateParams({ mat_page: newPage === 1 ? null : String(newPage) })
    }, [totalPages, updateParams])

    const handlePageSizeChange = useCallback((newSize: string) => {
        updateParams({ mat_size: newSize === '20' ? null : newSize, mat_page: null })
    }, [updateParams])
