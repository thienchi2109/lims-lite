-- CDC-LIMS Complete Database Setup
-- Fixed version that works with Supabase Postgres Docker image
-- Bypasses supabase_admin role requirement

-- ============================================================================
-- STEP 1: Enable Extensions (manually without role checks)
-- ============================================================================
DO $$
BEGIN
...
BEGIN
    IF (TG_OP = 'UPDATE') THEN
        INSERT INTO public.audit_logs (
            ...
            TG_TABLE_NAME, OLD.id, TG_OP, to_jsonb(OLD), to_jsonb(NEW), auth.uid()
        );
        RETURN NEW;
...
-- ============================================================================
-- STEP 9: Seed Data - Assay Definitions
-- ============================================================================
...
BEGIN
    FOR i IN 1..20 LOOP
        BEGIN
            ...
            RETURNING id INTO sample_id_val;
            
            created_count := created_count + 1;
        EXCEPTION
            WHEN unique_violation THEN
                -- Sample already exists, skip
                NULL;
        END;
    END LOOP;
...
-- ============================================================================
-- STEP 11: Assign Tests to First 5 Samples
-- ============================================================================
DO $$
DECLARE
    sample_rec RECORD;
...
SELECT 'Auth Users:' as status, COUNT(*)::text as count FROM auth.users;
SELECT 'Public Users:' as status, COUNT(*)::text as count FROM public.users;
SELECT 'Methods:' as status, COUNT(*)::text as count FROM public.methods;
SELECT 'Assays:' as status, COUNT(*)::text as count FROM public.assay_definitions;
SELECT 'Samples:' as status, COUNT(*)::text as count FROM public.samples;
SELECT 'Results (Pending):' as status, COUNT(*)::text as count FROM public.results WHERE status = 'pending';
...
\echo ''
\echo 'NOTE: Passwords are NOT set in the database.'
\echo 'You need to create these users via Supabase Auth/Studio.'
\echo '==================================='
