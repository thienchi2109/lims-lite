...
## What Changes
- Add `clients` table with required `name`, `date_of_birth` (DATE), `gender` (CHECK: Nam/Nữ/Khác), `id_card_num` (required, stored), **`phone` (required, NOT NULL, validated format)**, optional address/expiry/health insurance, and audit timestamps.
...
- **Affected code:** migrations (new table, constraints, policies, triggers), QR parsing/ingest server action, sample creation/assignment flows to require `client_id`, validation schemas/types, seeds/tests, UI forms (Vietnamese labels, QR auto-fill, phone validation).
- **Compliance:** soft delete unchanged (samples), audit coverage required for clients; follow migration security checklist.
- **Rationale for required phone:** Enables secure CoA (Certificate of Analysis) access via phone-based passcode (last 6 digits); aligns with healthcare industry standard for client contact.
