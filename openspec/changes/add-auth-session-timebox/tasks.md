## 1. Configuration
- [ ] 1.1 Define `SESSION_TIMEBOX_SECONDS` (default `14400`) for the Next.js app runtime
- [ ] 1.2 Document the new setting in `docs/DOCKER_SETUP.md` (and/or `env.md`)

## 2. Middleware Enforcement (Hard Timebox)
- [ ] 2.1 Create an edge-safe service-role Supabase client helper usable from middleware
- [ ] 2.2 Decode the access token in middleware and extract the `session_id` JWT claim
- [ ] 2.3 Query `auth.sessions` by `session_id` to get `created_at`
- [ ] 2.4 If `now - created_at > SESSION_TIMEBOX_SECONDS`: sign out, clear auth cookies, redirect to `/login?reason=session_expired`
- [ ] 2.5 Ensure existing role redirects still work (analyst vs manager)
