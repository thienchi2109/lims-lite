...
- **WHEN** an enabled ingestion or delivery goroutine exits unexpectedly or misses its configured heartbeat/progress threshold
- **THEN** readiness fails immediately or the process terminates for Docker restart
- **AND** liveness does not report a falsely healthy delivery pipeline indefinitely

### Requirement: SQLite backup and restore are operationally verified
The Notification Service SHALL use an online-safe backup procedure and SHALL periodically verify that backups can be restored.
...
- **AND** stores it outside the service data volume
...
- **AND** completes or safely releases in-progress outbox and delivery leases before the shutdown timeout
