...
The home-server deployment now separates the PDF path across two private Compose bridges. The application can reach only the authenticated `pdf-gateway` on `pdf-client`; only the gateway can reach raw Gotenberg on `pdf-upstream`. Neither service publishes a host port or participates in Nginx, Cloudflare Tunnel, or Tailscale routing, and neither may become a prerequisite for CoA creation or HTML viewing.
...
- Protect the home server with the application generation limiter plus gateway request, queue, concurrency, timeout, and container resource limits.
- Keep PDF download an optional capability so a gateway or Gotenberg outage does not affect CoA creation or HTML preview.
...
- Publishing the private gateway to another application or the Tailscale interface, onboarding a second consumer, or changing the existing LIMS-only gateway policy.
...
Each authorized request sends the validated HTML through the authenticated PDF gateway and streams the returned PDF to the browser. The application will not upload the PDF to Storage or write PDF metadata to `coa_reports`.
...
A client PDF request remains a narrow extension of the existing public CoA route. Service-role data access may begin only after the short-lived token establishes the client identity, must be scoped to the requested sample and report, and may only evaluate ownership, completed sample status, ready report status, confidential-sample concealment, and authorized artifact access. The confidential-association lookup uses that same scoped client rather than creating another admin client or calling a helper that logs raw database errors. Conversion may begin only after those checks pass. The PDF route does not introduce a reusable service-role repository, a generic RLS bypass, or broader client data access.
...
### 4. Submit the stored HTML through the authenticated PDF gateway

The application will send multipart form data to `POST /v1/convert/html` on the private gateway with the document named `index.html`. The conversion client constructs a new server-to-server request from an explicit header and form-field allowlist. It reads the LIMS-specific bearer credential from `PDF_GATEWAY_TOKEN_FILE` and sends that credential only to the gateway. It will not forward incoming `Cookie`, `Authorization`, proxy authorization, Supabase session headers, CoA tokens, service-role credentials, or public download URLs.
...
The application joins `pdf-client` and uses the compatibility setting `GOTENBERG_URL=http://pdf-gateway:8080`. The gateway joins `pdf-client` and `pdf-upstream`; raw Gotenberg joins only `pdf-upstream`. The application receives its plaintext LIMS bearer through the read-only `PDF_GATEWAY_TOKEN_FILE` Docker secret and must never receive the gateway client-policy secret.

Neither PDF service publishes a host port or appears in Nginx, Cloudflare Tunnel, or Tailscale routing. Gateway and Gotenberg health checks, restart policies, and resource limits remain independent of the application health check and startup dependency graph, so HTML workflows remain available during a converter outage.

### 8. Preserve the authenticated gateway contract

Gateway-specific request construction will live behind one server-only conversion client configured by `GOTENBERG_URL` and `PDF_GATEWAY_TOKEN_FILE`. Despite the compatibility variable name, the URL must identify the authenticated gateway, never raw Gotenberg. Route handlers and UI code depend on the application's PDF conversion contract, not on Compose service discovery or Gotenberg details.

The gateway currently serves only LIMS on the private Compose bridge. Publishing it over Tailscale, onboarding another consumer, or changing cross-application policy remains a separate change and must not weaken the LIMS credential, route, filename, or response contracts.
...
The limiter is checked after authorization and before calling the PDF gateway, so invalid unauthenticated requests do not populate it and authorized callers cannot consume unbounded Chromium work. A request that reaches the conversion boundary consumes one attempt even if the gateway or Gotenberg later fails.

The client route uses only Nginx's overwritten `X-Real-IP` value for the IP component. It ignores `X-Forwarded-For`, because the configured `$proxy_add_x_forwarded_for` chain preserves a caller-supplied leading value and is therefore not a trusted limiter key.

The single application-container deployment permits a bounded in-memory map for the MVP, matching the existing public authentication limiter pattern. The map has a hard cap of 10,000 keys. On insertion at capacity, it first removes expired windows. If all 10,000 remaining keys are still active, it rejects the new key with HTTP `429` and does not call the PDF gateway. It never evicts an active key because recreating that key would reset its counter and violate the five-attempt limit. Counters may reset on app restart. Distributed persistence is deferred until the app runs multiple instances.
...
After the short-lived token establishes a client identity, the route persists one terminal `coa_access_log` outcome before completing the response. For success, the authenticated gateway first returns valid PDF bytes, the success audit insert must then commit, and only then may the route deliver the PDF. If that insert fails, the route discards the generated bytes, returns a generic Vietnamese service-unavailable error, and emits a non-sensitive operational alert with a trace ID.

Ownership, status, confidentiality, storage, integrity, rate-limit, resource-loading, timeout, and conversion failures use a small allowlist of non-sensitive failure codes. Before the requested sample row is resolved, the failure audit keeps `sample_id` null so an unknown or malformed UUID cannot violate the audit table's foreign key; later failures use only the resolved row ID. The route attempts to persist the failure outcome before returning the failure response. If failure-audit persistence is unavailable, the request remains failed, the response becomes the same generic service-unavailable error, and an operational alert with the request-scoped trace ID is emitted without recursively attempting another audit insert. Audit and operational details must not contain sample identifiers, patient data, HTML, URLs, tokens, cookies, authorization headers, or service credentials.

No new staff read-audit persistence is added because equivalent staff HTML views are not currently read-audited. Operational logs may include a generated trace ID, route class, response status, duration, and gateway request ID, but must not include HTML, tokens, patient data, or service credentials.
...
- **[The gateway rejects authentication or the gateway/Gotenberg path is unreachable]** -> Return a Vietnamese PDF-specific error while leaving CoA creation and HTML preview operational; never fall back to raw Gotenberg.
- **[A future consumer requires a different gateway policy]** -> Keep the current LIMS transport and bearer handling in one configurable server-only client and defer multi-app policy to a separate change.
...
1. Use the existing private `pdf-client`/`pdf-upstream` topology, custom Gotenberg image, authenticated gateway, health checks, resource limits, `GOTENBERG_URL=http://pdf-gateway:8080`, and `PDF_GATEWAY_TOKEN_FILE` secret contract.
2. Before application rollout, verify the gateway authenticates the LIMS bearer, raw Gotenberg remains unreachable from the app, Times New Roman is installed, external logo/QR loading works, and both PDF containers remain bounded and unexposed.
3. Add and test shared report loading, hash verification, filename formatting, rate limiting, and authenticated gateway conversion utilities.
...
7. Deploy the application only after gateway-to-Gotenberg conversion is healthy, then verify HTML preview remains usable with the gateway or Gotenberg stopped.

Rollback removes the PDF UI action, routes, and conversion-client configuration. The already-shared gateway/Gotenberg infrastructure is managed by its own hardening change. No database or stored CoA rollback is required.
