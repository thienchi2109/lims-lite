...
- [ ] 3.5 Ghi commit boundary cho các primitive thuần, chưa gọi Gotenberg.

## Phase 4. Xây conversion client có thể thay gateway

- [ ] 4.1 Viết failing tests cho server-only Gotenberg client: `index.html`, `emulatedMediaType=print`, `printBackground=true`, `preferCssPageSize=true`, `skipNetworkIdleEvent=false`, `failOnResourceLoadingFailed=true` và `failOnResourceHttpStatusCodes=[400,599]`.
- [ ] 4.2 Viết failing tests cho timeout, non-PDF response, unavailable service, trace propagation, không log HTML/token và không forward cookie, auth header, CoA token, Supabase session hoặc service-role credential.
- [ ] 4.3 Implement client dùng native `fetch`/`FormData`, lấy endpoint từ `GOTENBERG_URL` và không để route/UI phụ thuộc trực tiếp vào Docker service name.
- [ ] 4.4 Xác nhận client có thể đổi base URL sang một future Tailscale PDF Gateway mà không đổi contract gọi.
- [ ] 4.5 Chạy focused tests và ghi commit boundary cho conversion client.

## Phase 5. Thêm luồng PDF cho staff

- [ ] 5.1 Viết failing route tests cho `GET /api/coa/view/pdf`: analyst, manager, doctor, role bị cấm, confidential access, hash mismatch và Gotenberg failure.
...
- [ ] 5.4 Kiểm tra filename `PhieuKetQuaXN-{samples.sample_id}-{YYYYMMDD}.pdf` từ `samples.sample_id` và `coa_reports.generated_at`; xác nhận request bị từ chối không gọi Gotenberg.
...
- [ ] 8.3 Chạy `rtk run "docker compose config --quiet"` và static tests xác nhận Gotenberg không có host port, Nginx/Tunnel exposure, có deny policy cho non-public IP và conversion client không forward auth material.
...
- [ ] 9.2 Build và start Gotenberg trên home server; xác nhận `/health`, `fc-match "Times New Roman"`, resource limits và không có host port.
...
- [ ] 9.5 Dừng Gotenberg trong cửa sổ kiểm chứng ngắn, xác nhận PDF báo lỗi tiếng Việt nhưng CoA creation và HTML preview vẫn hoạt động; sau đó khởi động lại và kiểm tra health.
- [ ] 9.6 Kiểm tra rate limit `5/10 phút`, bounded limiter, client success/failure access log, SSRF deny policy, trace không chứa dữ liệu nhạy cảm và không có PDF được lưu vào Storage/DB.
