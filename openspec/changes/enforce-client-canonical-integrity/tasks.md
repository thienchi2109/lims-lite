...
- [x] 1.1 Add `tests/client-canonical-integrity-preflight.sql` as a read-only
  ...
  mutation caller.
- [x] 1.2 Define the preflight predicates: trusted identity duplicates across
...
- [x] 1.3 Capture non-PII aggregate output in
...
- [x] 1.4 Verify migration 230 SHA-256
...
- [x] 1.5 Verify the existing client RLS policy set is unchanged:
...
- [x] 2.1 Verify migration 231 is unused in the repository and approved
  deployment checkout; if occupied, select the actual next number before
  creating a migration.
- [x] 2.2 Add a failing static contract test at
...
- [x] 2.3 Migrate `upsertClient()` so the route and shadow handler delegate to
...
- [x] 2.4 Implement canonical projection consistency guards and preserve
...
- [x] 2.5 Verify the exact post-230 ACL: authenticated has no table-level
...
- [x] 3.1 Add `tests/client-canonical-integrity-gate.test.sql` as rollback-only
...
- [x] 3.2 Extend or add a multi-session concurrency rehearsal based on
...
- [x] 3.3 Assert one explicit lifecycle audit event per successful operation
...
- [x] 3.4 Run the preflight, `run_security_tests()`, the named SQL suites,
...
- [x] 3.5 Apply only the committed next-numbered migration in the approved
  Docker environment and verify catalog, ACL, RLS, RPC metadata, audit evidence,
  and unchanged sample/result history.
- [x] 3.6 Record the forward-only rollback rehearsal and Gate A release
