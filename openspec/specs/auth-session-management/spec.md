...
## Purpose
Enforce an absolute maximum lifetime for authenticated sessions (hard timebox) and provide consistent user experience when sessions expire.
...
- **THEN** the system SHALL:
  ...
  - Redirect the user to `/login?reason=session_expired`
...
- **THEN** the application SHALL force sign-out and redirect the user to `/login?reason=session_expired`.

### Requirement: Session timebox is configurable
The system SHALL allow operators to configure the maximum session lifetime via environment configuration, with a secure default of 4 hours.
