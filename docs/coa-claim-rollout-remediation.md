...
SELECT set_config(
    'lims.coa_forward_input_correction',
    :'correction_reason',
    false
);
SELECT set_config(
    'lims.coa_forward_input_approval_reference',
    :'approval_reference',
    false
);
DO $validate_inputs$
BEGIN
    IF NULLIF(BTRIM(current_setting(
        'lims.coa_forward_input_correction'
    )), '') IS NULL THEN
        RAISE EXCEPTION 'correction_reason must not be blank';
    END IF;
    IF NULLIF(BTRIM(current_setting(
        'lims.coa_forward_input_approval_reference'
    )), '') IS NULL THEN
        RAISE EXCEPTION 'approval_reference must not be blank';
    END IF;
END;
$validate_inputs$;
BEGIN;
