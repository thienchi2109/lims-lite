-- Migration: 125_add_confidentiality_to_assay_rpcs.sql
-- Purpose: Return assay confidentiality flags from assay RPCs so reads fail closed.
-- Security impact:
--   - No RLS or role policy changes
--   - Preserves existing authenticated EXECUTE grants
--   - Exposes assay_definitions.is_confidential through existing assay RPCs

DROP FUNCTION IF EXISTS get_assay_definitions(text, uuid, uuid, int, int);

CREATE FUNCTION get_assay_definitions(
...
COMMENT ON FUNCTION get_assay_definitions IS
    'Returns paginated assay definitions with specialty info, confidentiality state, and aggregated methods. Supports search by name/specialty/method, and filter by specialty_id or method_id.';

DROP FUNCTION IF EXISTS get_assay_definition_by_id(uuid);

CREATE FUNCTION get_assay_definition_by_id(p_id uuid)
RETURNS TABLE (
    id uuid,
    name text,
    specialty_id uuid,
...
COMMENT ON FUNCTION get_assay_definition_by_id IS
    'Returns a single assay definition by ID with confidentiality state and aggregated methods array.';
