-- Allow the audited sample label print RPC to record the printer stock thermal label preset.
--
-- Security Impact:
...
CREATE OR REPLACE FUNCTION public.record_sample_label_print(
    ...
    p_label_preset TEXT DEFAULT 'thermal-35x23-sheet-2up'
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
    ...
    v_preset TEXT := COALESCE(NULLIF(BTRIM(p_label_preset), ''), 'thermal-35x23-sheet-2up');
BEGIN
