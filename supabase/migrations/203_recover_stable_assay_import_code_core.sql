...
BEGIN
    IF to_regclass('public.assay_definitions') IS NULL THEN
        RAISE EXCEPTION
            'Migration 203 requires public.assay_definitions';
    END IF;
END;
$table_baseline$;
...
BEGIN
    IF to_regclass('public.assay_import_code_seq') IS NOT NULL THEN
        RAISE EXCEPTION
            'Migration 203 expected public.assay_import_code_seq to be absent';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'Migration 203 expected assay_definitions.import_code to be absent';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 203 found an incompatible assay_definitions baseline';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 203 found a partial assay import-code contract';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 203 requires enabled assay timestamp and audit triggers';
    END IF;
    ...
    IF v_assay_count > 999999 THEN
        RAISE EXCEPTION
            'Migration 203 cannot encode % assays in six digits',
            v_assay_count;
    END IF;
END;
$baseline$;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 203 backfill verification failed: null %, soft-deleted null %, invalid %, duplicate %',
            v_null_count,
            v_soft_deleted_null_count,
            v_invalid_count,
            v_duplicate_count;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION
            'Migration 203 sequence contract verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 203 sequence position verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 203 exposed direct sequence allocation to an API role';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 203 schema or trigger verification failed';
    END IF;
END;
$verification$;
