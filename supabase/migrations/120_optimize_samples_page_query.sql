-- Migration 120: Optimize samples list query path
-- Security Impact: Low - adds a read-only index and a SECURITY INVOKER RPC that preserves existing RLS checks
-- Purpose: Collapse samples listing into one paginated RPC and remove extra application round-trips

SET search_path TO public;

-- Support the default samples sort without scanning all active rows first.
CREATE INDEX IF NOT EXISTS idx_samples_updated_at_not_deleted
ON samples(updated_at DESC)
WHERE deleted_at IS NULL;

-- Drop first so repeated applies stay idempotent during local development.
...
COMMENT ON FUNCTION get_samples_page(
    ...
    INTEGER,
    INTEGER
) IS 'Returns paginated sample rows plus total_count in a single RPC. Preserves RLS with SECURITY INVOKER and supports the Samples screen filters.';
