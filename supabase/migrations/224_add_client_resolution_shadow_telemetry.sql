-- Migration 224: Add deterministic client-resolution shadow telemetry.
--
-- Security impact:
...
--
-- Rollout impact:
-- - The RPC evaluates legacy and v2 decisions in one pre-mutation transaction.
-- - It never inserts, updates, deletes, merges, or relinks clients or samples.
-- - Telemetry expires after 30 days and expired rows are pruned on every write.
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 224 requires the complete Phase 4 resolver baseline';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'Migration 224 appears to have been applied already';
    END IF;
END;
$baseline$;
...
DECLARE
    ...
    v_legacy_client RECORD;
    v_phone_client RECORD;
    v_v2_result RECORD;
...
BEGIN
    ...
    IF p_caller_category IN ('manual', 'qr') THEN
        SELECT
            client.id,
            client.deleted_at
        INTO v_legacy_client
        FROM public.clients AS client
        WHERE client.name = v_trimmed_name
          AND client.date_of_birth = p_date_of_birth
        LIMIT 1;

        IF v_legacy_client.id IS NULL
           OR (
                NOT v_can_access_confidential
                AND public.client_has_confidential_samples(v_legacy_client.id)
           )
        THEN
    ...
    ELSE
        ...
        IF v_phone_client.id IS NOT NULL
           AND (
                lower(v_phone_client.name) <> lower(v_trimmed_name)
                OR v_phone_client.date_of_birth <> p_date_of_birth
           )
        THEN
        ...
        ELSE
            ...
            IF v_legacy_client.id IS NULL THEN
                v_legacy_outcome := 'not_found';
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 224 telemetry column whitelist failed: %',
            v_columns;
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'Migration 224 deny policy verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'Migration 224 RPC grants verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'Migration 224 direct table grants verification failed';
    END IF;
END;
$verify$;
