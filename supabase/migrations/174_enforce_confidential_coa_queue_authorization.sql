-- Migration 174: Enforce confidential authorization in CoA queue claims
-- Security Impact: High
-- Changes:
--   - Replaces the authenticated SECURITY DEFINER queue RPC.
...
BEGIN
    IF to_regprocedure(
        ...
        RAISE EXCEPTION
            'Migration 174 requires user_can_access_confidential()';
    END IF;
    ...
    IF to_regprocedure(
        ...
        RAISE EXCEPTION
            'Migration 174 requires the migration 171 queue RPC';
    END IF;
    ...
    IF v_queue_definition NOT ILIKE '%generation_claimed_at%'
       OR v_queue_definition NOT ILIKE '%15 minutes%'
       OR v_queue_definition NOT ILIKE
           '%v_snapshot_count <> v_result_count%' THEN
        RAISE EXCEPTION
            'Migration 174 found an unexpected queue RPC baseline';
    END IF;

    IF v_queue_definition ILIKE
       '%public.user_can_access_confidential()%' THEN
        RAISE EXCEPTION
            'Migration 174 expected the queue RPC to lack confidential authorization';
    END IF;
END;
$$;
...
IS 'Claims a CoA generation lease after role, confidentiality, and locked provenance validation.';
