-- Migration 151: Register analyst OTP Phase 1 security test
-- Security Impact: Verification-only
-- Adds analyst OTP prerequisites to the mandatory run_security_tests() suite.
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE WARNING 'SECURITY TEST FAILED: analyst OTP settings manager policies are missing';
        RETURN FALSE;
    END IF;
