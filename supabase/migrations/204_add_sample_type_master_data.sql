-- Migration 204: Add sample-type master data foundation.
--
-- Security impact:
...
BEGIN
    IF to_regclass('public.samples') IS NULL THEN
        RAISE EXCEPTION 'Migration 204 requires public.samples';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 found an existing sample-type master-data object';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'Migration 204 expected samples.sample_type_id to be absent';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 found an incompatible samples baseline';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 requires audit, timestamp, and role helpers';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 requires exact samples audit/timestamp triggers';
    END IF;
END;
$table_baseline$;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 found a partial sample-type function contract';
    END IF;
END;
$function_baseline$;
...
BEGIN
    ...
    IF v_blank_count <> 0 OR v_collision_count <> 0 THEN
        RAISE EXCEPTION
            'Migration 204 cannot map sample types: blank %, collisions %',
            v_blank_count,
            v_collision_count;
    END IF;
    ...
    IF v_sample_type_count > 999999 THEN
        RAISE EXCEPTION
            'Migration 204 exceeds the LM-NNNNNN code capacity';
    END IF;
END;
$historical_baseline$;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 requires the existing assignment RPC contracts';
    END IF;
    ...
    PERFORM set_config(
        'lims.migration_204_accession_definition',
        v_accession_definition,
        true
    );
    PERFORM set_config(
        'lims.migration_204_assignment_definition',
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 sample-type verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 sample backfill verification failed';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION
            'Migration 204 sequence contract verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 sequence position verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 exposed sequence allocation to an API role';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 table privilege verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 trigger binding verification failed';
    END IF;
END;
$verification$;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 204 changed an assignment RPC contract';
    END IF;
END;
