-- Migration 024: Assign tests RPC with status updates

SET search_path TO public;

DROP FUNCTION IF EXISTS public.assign_tests_to_sample(UUID, JSONB);
...
BEGIN
    ...
    WITH expanded AS (
        SELECT
            ...
            (test->>'methodId')::uuid AS method_id
...
REVOKE ALL ON FUNCTION public.assign_tests_to_sample(UUID, JSONB) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.assign_tests_to_sample(UUID, JSONB) TO authenticated;
