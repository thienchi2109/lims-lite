-- Migration 164: Register assessment snapshot SELECT policy security test
-- Security Impact: Low
-- Changes:
--   - Verifies the snapshot table has exactly one expected SELECT policy.
--   - Verifies the policy is limited to authenticated analysts and managers.
--   - Registers the contract in the shared run_security_tests() helper.

SET search_path TO public;

CREATE OR REPLACE FUNCTION public.test_assessment_snapshot_select_policy_guard()
RETURNS BOOLEAN
LANGUAGE plpgsql
SET search_path = public, pg_temp
AS $$
DECLARE
    v_authenticated_role OID := 'authenticated'::regrole;
...
BEGIN
    ...
    IF v_policy_count <> 1 THEN
        RAISE WARNING
            'SECURITY TEST FAILED: expected exactly one assessment snapshot SELECT policy, found %',
            v_policy_count;
        RETURN FALSE;
    END IF;
    ...
    FROM pg_catalog.pg_policy
    WHERE polrelid = 'public.result_reference_assessments'::regclass
      AND polname = 'Analysts and managers can view assessment snapshots'
      AND polcmd = 'r';

    IF v_policy_expression IS NULL THEN
        RAISE WARNING
            'SECURITY TEST FAILED: expected assessment snapshot SELECT policy is missing';
        RETURN FALSE;
    END IF;

    IF v_policy_roles IS DISTINCT FROM ARRAY[v_authenticated_role] THEN
        RAISE WARNING
            'SECURITY TEST FAILED: assessment snapshot SELECT policy roles are %, expected authenticated only',
            v_policy_roles;
        RETURN FALSE;
    END IF;

    IF v_policy_expression NOT ILIKE '%get_user_role()%'
       OR v_policy_expression NOT ILIKE '%manager%'
       OR v_policy_expression NOT ILIKE '%analyst%'
       OR v_policy_expression NOT ILIKE '%sample_submissions%'
       OR v_policy_expression NOT ILIKE '%result_reference_assessments.submission_id%'
       OR v_policy_expression NOT ILIKE '%submission.user_id%'
       OR v_policy_expression NOT ILIKE '%auth.uid()%' THEN
        RAISE WARNING
            'SECURITY TEST FAILED: assessment snapshot SELECT policy lost required role or ownership restrictions. Policy: %',
            v_policy_expression;
        RETURN FALSE;
    END IF;
...
DO $$
BEGIN
    IF NOT public.test_assessment_snapshot_select_policy_guard() THEN
        RAISE EXCEPTION
            'Assessment snapshot SELECT policy security contract verification failed';
    END IF;
END;
$$;
...
REVOKE ALL ON FUNCTION public.test_assessment_snapshot_select_policy_guard()
FROM PUBLIC;
REVOKE ALL ON FUNCTION public.run_security_tests()
FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.test_assessment_snapshot_select_policy_guard()
TO authenticated;
GRANT EXECUTE ON FUNCTION public.run_security_tests()
TO authenticated;

COMMENT ON FUNCTION public.test_assessment_snapshot_select_policy_guard()
IS 'Verifies the assessment snapshot SELECT policy preserves authenticated analyst ownership and manager visibility restrictions.';
COMMENT ON FUNCTION public.run_security_tests()
IS 'Runs security verification tests, including the assessment snapshot SELECT policy contract.';
