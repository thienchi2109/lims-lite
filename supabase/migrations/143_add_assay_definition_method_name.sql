-- Add assay-owned method text to assay definitions.
--
-- Security impact:
-- - No RLS policies are changed.
-- - Adds nullable TEXT metadata managed through existing manager-only assay actions.
-- - Recreates assay read RPCs with the existing authenticated EXECUTE grant and explicit SECURITY DEFINER search_path.
-- - Keeps legacy methods/assay_methods only as compatibility and suggestion sources.
...
COMMENT ON COLUMN public.assay_definitions.method_name IS
    'Assay-owned method text managed by managers; legacy methods and assay_methods are compatibility/suggestion sources only.';

UPDATE public.assay_definitions AS assay
SET method_name = (
    SELECT method.name
    FROM public.assay_methods AS assay_method
    JOIN public.methods AS method
        ON method.id = assay_method.method_id
    WHERE assay_method.assay_id = assay.id
      AND method.deleted_at IS NULL
    ORDER BY assay_method.is_default DESC, method.name
    LIMIT 1
)
WHERE assay.method_name IS NULL
...
COMMENT ON FUNCTION public.get_assay_definitions(text, uuid, uuid, int, int) IS
    'Returns paginated assay definitions with assay-owned method text, specialty info, confidentiality state, and legacy aggregated methods for compatibility.';
...
COMMENT ON FUNCTION public.get_assay_definition_by_id(uuid) IS
    'Returns one assay definition with assay-owned method text, confidentiality state, and legacy aggregated methods for compatibility.';
