-- Migration 225: Supersede failed rehearsal migration 224.
--
-- Migration 224 is immutable after execution against the Phase 5 rehearsal
-- database. It failed before creating any object because that rehearsal
-- database was owned by supabase_admin, so the production apply role postgres
-- did not inherit CREATE on the pg_database_owner-owned public schema.
--
-- Security impact:
...
-- - Telemetry expires after 30 days and expired rows are pruned on each write.
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 225 requires the complete Phase 4 resolver baseline';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 225 requires the failed migration 224 no-object baseline';
    END IF;

    IF NOT has_schema_privilege(current_user, 'public', 'CREATE') THEN
        RAISE EXCEPTION
            'Migration 225 apply role requires CREATE on schema public';
    END IF;
END;
$baseline$;
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 225 telemetry column whitelist failed: %',
            v_columns;
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'Migration 225 deny policy verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'Migration 225 RPC grants verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION 'Migration 225 direct table grants verification failed';
    END IF;
END;
$verify$;
