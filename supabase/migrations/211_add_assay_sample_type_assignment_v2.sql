-- Migration 211: Add fail-closed assay/sample-type resolver and assignment v2.
--
-- Security impact: adds two internal SECURITY DEFINER resolvers with no client
-- EXECUTE grant and three authenticated-only additive RPCs. Existing security
-- checker functions are extended to cover the new signatures and fixed
-- search_path requirements.
-- Historical data impact: no existing sample, result, audit row, catalog
-- revision, or legacy assignment RPC is changed. Compatibility is checked only
-- for writes through the additive v2 functions; Phase 8 trigger enforcement is
...
DO $baseline$
DECLARE
    v_execute_checker REGPROCEDURE :=
        to_regprocedure('public.test_security_definer_rpc_execute_privileges()');
    v_search_path_checker REGPROCEDURE :=
        to_regprocedure('public.test_security_definer_rpc_search_path()');
    v_analyst_role_checker REGPROCEDURE :=
        to_regprocedure('public.test_sample_accession_rpcs_require_analyst_role()');
    v_execute_definition TEXT;
    v_search_path_definition TEXT;
    v_analyst_role_definition TEXT;
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 211 requires the Phase 1-4 compatibility schema';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION
            'Migration 211 compatibility columns do not match the expected baseline';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 211 requires all current legacy assignment RPCs';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 211 expected resolver and assignment v2 functions to be absent';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION
            'Migration 211 requires the existing sample and result audit triggers';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 211 requires the registered security test functions';
    END IF;

    SELECT pg_get_functiondef(v_execute_checker)
    INTO v_execute_definition;
    SELECT pg_get_functiondef(v_search_path_checker)
    INTO v_search_path_definition;
    SELECT pg_get_functiondef(v_analyst_role_checker)
    INTO v_analyst_role_definition;

    IF v_execute_definition NOT ILIKE
           '%create_sample_atomic(uuid,text,timestamp with time zone,uuid,text,boolean)%'
       OR v_execute_definition NOT ILIKE
           '%accession_and_assign_tests(uuid,text,timestamp with time zone,jsonb,text,boolean)%'
       OR v_search_path_definition NOT ILIKE
           '%create_sample_atomic(uuid,text,timestamp with time zone,uuid,text,boolean)%'
       OR v_analyst_role_definition NOT ILIKE
           '%v_user_role <> ''analyst''%'
    THEN
        RAISE EXCEPTION
            'Migration 211 security checker baseline is not the quality-aware contract';
    END IF;
END;
$baseline$;
...
DECLARE
    ...
    v_service_role_denied_functions TEXT[] := ARRAY[
        'public.resolve_sample_type_compatibility_revision(uuid,bigint)',
        'public.resolve_assay_sample_type_compatibility(uuid,uuid,bigint)',
        'public.create_sample_atomic_v2(uuid,text,timestamp with time zone,uuid,uuid,boolean,bigint)',
        'public.accession_and_assign_tests_v2(uuid,text,timestamp with time zone,jsonb,uuid,boolean,bigint)',
...
BEGIN
    ...
    FOREACH v_function IN ARRAY v_service_role_denied_functions LOOP
        IF has_function_privilege(
            'service_role',
            v_function,
            'EXECUTE'
...
BEGIN
    ...
    WHERE function_record.pronamespace = 'public'::REGNAMESPACE
      AND function_record.oid::REGPROCEDURE::TEXT IN (
          'create_sample_atomic(uuid,text,timestamp with time zone,uuid,text,boolean)',
          'accession_and_assign_tests(uuid,text,timestamp with time zone,jsonb,text,boolean)',
          'resolve_sample_type_compatibility_revision(uuid,bigint)',
          'resolve_assay_sample_type_compatibility(uuid,uuid,bigint)',
          ...
          'assign_tests_to_sample_v2(uuid,uuid,jsonb,bigint)'
      )
      AND NOT EXISTS (
          SELECT 1
          FROM unnest(
          ...
          WHERE configuration = 'search_path=public, extensions'
      );

    IF v_missing_count <> 0 THEN
        RAISE WARNING
            'SECURITY TEST FAILED: % assignment SECURITY DEFINER RPC(s) lack fixed search_path',
            v_missing_count;
        RETURN FALSE;
    END IF;
...
DECLARE
    ...
    v_create_v2_def TEXT;
    v_accession_v2_def TEXT;
BEGIN
    ...
    IF v_accession_def IS NULL
       ...
       OR v_accession_v2_def IS NULL
       OR v_accession_v2_def NOT ILIKE
          '%v_user_role IS DISTINCT FROM ''analyst''%'
       OR v_accession_v2_def ILIKE '%manager%'
    THEN
        RAISE WARNING
            'SECURITY TEST FAILED: accession RPC is not analyst-only';
        RETURN FALSE;
    END IF;
...
BEGIN
    FOREACH v_function IN ARRAY ARRAY[
        'public.resolve_sample_type_compatibility_revision(uuid,bigint)'::REGPROCEDURE,
        'public.resolve_assay_sample_type_compatibility(uuid,uuid,bigint)'::REGPROCEDURE,
        'public.create_sample_atomic_v2(uuid,text,timestamp with time zone,uuid,uuid,boolean,bigint)'::REGPROCEDURE,
        'public.accession_and_assign_tests_v2(uuid,text,timestamp with time zone,jsonb,uuid,boolean,bigint)'::REGPROCEDURE,
    ...
    LOOP
        ...
        IF NOT v_security_definer
           OR NOT (
               'search_path=public, extensions' = ANY(v_settings)
           )
        THEN
            RAISE EXCEPTION
                'Migration 211 function security verification failed for %',
                v_function;
        END IF;
    END LOOP;
    ...
    IF has_function_privilege(
        ...
        RAISE EXCEPTION
            'Migration 211 forbidden EXECUTE privilege verification failed';
    END IF;
    ...
    IF NOT has_function_privilege(
        ...
        RAISE EXCEPTION
            'Migration 211 authenticated EXECUTE grant verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 211 registered security checker verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 211 legacy assignment compatibility verification failed';
    END IF;
    ...
    IF EXISTS (
        ...
        RAISE EXCEPTION
            'Migration 211 must not add Phase 8 result enforcement';
    END IF;
END;
$verification$;
