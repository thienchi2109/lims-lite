-- Migration 193: Add dark approval-batch storage.
--
-- Security impact:
...
-- - Migration 192 remains immutable and is only validated as the baseline.
...
BEGIN
    ...
    THEN
        RAISE EXCEPTION
            'Migration 193 found an unexpected Phase P2 baseline';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 193 approval batch relations already exist';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 193 approval batch functions already exist';
    END IF;

    IF NOT public.test_atomic_result_approval_rpc_security() THEN
        RAISE EXCEPTION
            'Migration 193 requires the Phase P2 atomic approval contract';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 193 found an unexpected run_security_tests baseline';
    END IF;
END;
$baseline$;
...
BEGIN
    ...
    IF EXISTS (
        WITH expected(constraint_name, required_text) AS (
            VALUES
                ('approval_batches_parent_not_self_check'::TEXT, '%parent_batch_id <> id%'::TEXT),
                ('approval_batches_request_mode_check', '%retry_failed%'),
                ('approval_batches_parent_mode_check', '%parent_batch_id IS NOT NULL%'),
                ('approval_batches_request_fingerprint_check', '%[0-9a-f]{64}%'),
                ('approval_batches_approval_note_check', '%char_length(approval_note)%'),
                ('approval_batches_step_up_cohort_check', '%manager_email_otp%'),
                ('approval_batches_step_up_time_check', '%15 minutes%'),
                ('approval_batches_status_check', '%completed_with_failures%'),
                ('approval_batches_status_timestamps_check', '%completed_at IS NOT NULL%'),
                ('approval_batch_items_snapshot_check', '%approval_batch_uuid_array_is_canonical%'),
                ('approval_batch_items_status_check', '%retry_wait%'),
                ('approval_batch_items_attempt_count_check', '%BETWEEN 0 AND 3%'),
                ('approval_batch_items_claim_check', '%claim_expires_at > claimed_at%'),
                ('approval_batch_items_error_code_check', '%[A-Z][A-Z0-9_]{1,79}%'),
                ('approval_batch_items_error_params_check', '%approval_batch_error_params_are_safe%'),
                ('approval_batch_items_state_check', '%status = ''failed''%'),
                ('approval_batch_item_attempts_attempt_number_check', '%BETWEEN 1 AND 3%'),
                ('approval_batch_item_attempts_event_type_check', '%terminal_failure%'),
                ('approval_batch_item_attempts_error_code_check', '%[A-Z][A-Z0-9_]{1,79}%'),
                ('approval_batch_item_attempts_error_params_check', '%approval_batch_error_params_are_safe%')
        )
        SELECT 1
        FROM expected
        ...
        WHERE constraint_record.oid IS NULL
           OR pg_get_constraintdef(constraint_record.oid)
                NOT ILIKE expected.required_text
    ) THEN
        RETURN FALSE;
    END IF;
    ...
    IF (
       ...
       OR EXISTS (
           WITH expected(index_name, table_name, required_text) AS (
               VALUES
                   ('approval_batches_requested_by_created_at_idx'::TEXT, 'approval_batches'::TEXT, '%(requested_by, created_at DESC, id)%'::TEXT),
                   ('approval_batches_parent_batch_id_idx', 'approval_batches', '%(parent_batch_id)%WHERE (parent_batch_id IS NOT NULL)%'),
                   ('approval_batch_items_batch_status_idx', 'approval_batch_items', '%(batch_id, status)%'),
                   ('approval_batch_items_batch_id_id_idx', 'approval_batch_items', '%(batch_id, id)%'),
                   ('approval_batch_items_sample_id_idx', 'approval_batch_items', '%(sample_id)%'),
                   ('approval_batch_item_attempts_item_time_idx', 'approval_batch_item_attempts', '%(batch_item_id, attempt_number, occurred_at)%')
           )
           SELECT 1
           FROM expected
           LEFT JOIN pg_indexes AS index_record
             ON index_record.schemaname = 'public'
            AND index_record.indexname = expected.index_name
            AND index_record.tablename = expected.table_name
           WHERE index_record.indexname IS NULL
              OR index_record.indexdef NOT ILIKE expected.required_text
       )
       OR (
           SELECT count(*)
           FROM pg_indexes
...
BEGIN
    IF NOT public.test_atomic_result_approval_rpc_security() THEN
        RAISE EXCEPTION
            'Migration 193 atomic approval baseline verification failed';
    END IF;

    IF NOT public.approval_batch_storage_catalog_is_exact() THEN
        RAISE EXCEPTION
            'Migration 193 exact storage catalog verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 193 storage catalog verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 193 storage privilege verification failed';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 193 must leave approval batch contracts absent';
    END IF;
    ...
    THEN
        RAISE EXCEPTION
            'Migration 193 changed the Phase P2 security registration';
    END IF;
END;
$verification$;
